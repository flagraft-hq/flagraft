    ...
    <div>
      ...
      <div className="grid-2">
        {envs.map((e) => (
          <div key={e.slug} className="card" style={{ padding: 20, position: 'relative' }}>
            ...
            <div
              className="mono"
              style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 14 }}
            >
              /{e.slug}
            </div>
            ...
            <div className="grid-3" style={{ gap: 8 }}>
              {[
                ...
                <div
                  key={s.l}
                  style={{ padding: '10px 12px', background: 'var(--bg-subtle)', borderRadius: 8 }}
                >
      ...
      <Modal open={showNew} onClose={() => setShowNew(false)}>
        <div className="modal-header">
          ...
          <div className="sub">
            Cloned from <span className="mono">production</span> defaults.
          </div>
        </div>
        <div className="modal-body">
          <div className="field">
            <label>Name</label>
            <input className="input" placeholder="e.g. Preview" autoFocus />
          </div>
          <div className="field">
            <label>Slug</label>
            <input className="input mono" placeholder="preview" />
            <span className="hint">Used in URLs and SDK config. Lowercase, no spaces.</span>
          </div>
...
function KeysScreen() {
    ...
    <div>
                  ...
                  {k.project ? (
                    <div className="muted" style={{ fontSize: 12 }}>
                      ...
                      {k.env ? (
                        <>
                          {' '}
                          · <span className="mono">{k.env}</span>
                        </>
                      ) : null}
                    </div>
                  ) : null}
      ...
      <div
        style={{
          ...
          border: '1px solid var(--acc-soft-border)',
          borderRadius: 8,
          display: 'grid',
      ...
      >
        ...
        <div style={{ fontSize: 12.5 }}>
          <b style={{ color: 'var(--acc-fg)' }}>Keys are shown once.</b> Flagraft only stores a
          hash, so we can't recover the plaintext. Rotate immediately if a key leaks — old keys can
          be revoked while the new one is issued.
        </div>
      </div>
      ...
      <Modal open={showNew} onClose={() => setShowNew(false)}>
        <div className="modal-header">
          <h2>Issue API key</h2>
          <div className="sub">
            The plaintext key is shown <b>only once</b> after creation.
          </div>
        </div>
        <div className="modal-body">
          <div className="field">
          ...
          <div className="field">
            ...
            <span className="hint">
              Admin keys can read and write flags. Client keys only evaluate.
            </span>
          </div>
          <div className="field-row">
            <div className="field">
        ...
        <div className="modal-footer">
          ...
          <Button
            ...
            onClick={() => {
              setShowNew(false)
              setRevealKey(
                'ff_ad_' + Math.random().toString(36).slice(2, 8) + 'd1f8b3a02e7c4a17e9f2',
              )
            }}
          >
            Generate key
      ...
      <Modal open={!!revealKey} onClose={() => setRevealKey(null)}>
        <div className="modal-header">
          ...
          <div className="sub">
            You won't be able to see it again. Closing this dialog is the same as the key being
            lost.
          </div>
        </div>
        <div className="modal-body">
          ...
          >
            ...
            <button
              className="btn sm"
              style={{ background: '#172238', color: '#e4f7f3', border: '1px solid #2a3855' }}
              onClick={() => {
          ...
          <div
            style={{ marginTop: 14, padding: 12, background: 'var(--bg-subtle)', borderRadius: 8 }}
          >
            <div className="row" style={{ marginBottom: 6, fontSize: 12.5, fontWeight: 600 }}>
  ...
  -H "Authorization: ${revealKey || ''}"`}
        ...
        <div className="modal-footer">
          <span className="muted" style={{ fontSize: 12 }}>
            Stored as a SHA-256 hash. Plaintext is not recoverable.
          </span>
          <span className="spacer" />
          <Button
            variant="primary"
            onClick={() => {
              setRevealKey(null)
              toast.push({ title: 'Key created', msg: 'Make sure you saved the plaintext.' })
            }}
          >
            I've saved it
          </Button>
...
function AuditScreen() {
  ...
  const grouped = useMemo(() => {
    const xs = window.AUDIT.filter(
      (a) => (actor === 'all' || a.actor === actor) && (env === 'all' || a.env === env),
    )
    const m = {}
    xs.forEach((a) => {
      if (!m[a.day]) m[a.day] = []
  ...
  return (
    <div>
      <PageHeader
        title="Audit log"
        sub="Every state-changing action across this project — actor, when, what."
        actions={
          <>
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: r.desc.replace(
                            /`([^`]+)`/g,
                            '<span class="mono" style="font-size:12.5px;color:var(--text-1)">$1</span>',
                          ),
                        }}
                      />
                      <div className="actor mono">
                        by {r.actor}
                        {r.env ? <> · {r.env}</> : null}
                      </div>
                    </div>
  ...
  const steps = [
    {
      ...
      body: (
        <>
          You're running <b>v1.4</b> on Postgres. Let's walk through the four things you'll need to
    ...
    },
    {
      ...
      body: (
        <>
          One project per application. Each project gets <span className="mono">development</span>,{' '}
          <span className="mono">staging</span>, and <span className="mono">production</span>{' '}
      ...
      content: (
        <div className="field" style={{ marginTop: 14 }}>
          ...
          <div className="hint">
            Slug: <span className="mono">checkout-web</span> · auto-generated
          </div>
        </div>
      ),
      next: 'Create project',
    },
    {
      ...
      body: (
        <>
          Admin keys let your team manage flags in this project. The root key (from{' '}
          <span className="mono">pnpm admin:create-root-key</span>) stays in your password manager.
        </>
      ),
      content: (
...
Copy this — it won't be shown again.`}</pre>
    ...
    },
    {
      ...
      body: (
        <>
          Flags start off in every environment. Wire your code first, deploy, then flip in staging
          when you're ready.
        </>
      ),
      content: (
        <pre
          className="code"
          style={{ marginTop: 14 }}
        >{`if (await ff.isEnabled('hello-world', { userId })) {
  // new behaviour
}`}</pre>
      ),
      next: 'Create flag',
    },
    {
      ...
      body: (
        <>
          Open the Flags page to see your new flag. From there, toggle per environment, add
          overrides, and watch evaluations roll in.
        </>
      ),
    ...
    <div style={{ display: 'grid', placeItems: 'center', minHeight: '70vh' }}>
      <div className="card" style={{ width: 560, padding: 0 }}>
              ...
              >
                <span
                  style={{
                    ...
                    placeItems: 'center',
                    background:
                      i < step ? 'var(--pri)' : i === step ? 'var(--pri-soft)' : 'var(--bg-subtle)',
              ...
              {i < steps.length - 1 ? (
                <div
                  style={{
                    flex: 1,
                    height: 1,
                    background: i < step ? 'var(--pri)' : 'var(--border)',
                  }}
                />
              ) : null}
            </React.Fragment>
        ...
        <div className="modal-footer">
          ...
          >
            {cur.next} <Icon name="arrowRight" size={12} />
          </Button>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid-2">
        <ErrorCard
          ...
          cta={
            <>
              ...
              <Button variant="ghost">Run /ready check</Button>
            </>
          }
          meta={
            <>
              Last contact <span className="mono">12s ago</span> · auto-retrying every{' '}
              <span className="mono">5s</span>
            </>
          }
        />
        ...
        <ErrorCard
          ...
          msg={
            <>
              <span className="mono">GET /ready</span> returned{' '}
              <b className="mono" style={{ color: 'var(--danger-fg)' }}>
                503
              </b>
              . Reads continue from the in-process cache; writes are paused until the DB returns.
            </>
          }
          cta={
          ...
          meta={
            <>
              Affected: <span className="mono">writes</span> · cache TTL{' '}
        ...
        <ErrorCard
          ...
          meta={
            <>
              Current scope: <span className="mono">client / production</span>
            </>
          }
        />
        ...
        <ErrorCard
          ...
          msg={
            <>
              You hit <b className="num">100</b> client evaluations per minute from this IP.
              Existing in-process SDK caches still serve. New requests get{' '}
          ...
          meta={
            <>
              Window resets in <span className="mono num">00:42</span>
            </>
          }
        />
        ...
        <ErrorCard
          ...
          msg={
            <>
              Showing data from <span className="mono">14:32:08</span>. Your browser tab was
              backgrounded — the live socket reconnects automatically when you return.
            </>
          }
          cta={
            <Button variant="ghost" leftIcon="refresh">
          ...
          meta={
            <>
              Drift: <span className="mono num">+2m 14s</span>
            </>
          }
        />
        ...
        <ErrorCard
          ...
          msg={
            <>
              The flag key <span className="mono">checkout new cart</span> contains a space. Keys
              must match <span className="mono">^[a-z0-9._-]+$</span>.
            </>
          }
          cta={<Button variant="primary">Fix key</Button>}
          meta={
            <>
              Hint: <span className="mono">checkout.new-cart</span>
            </>
          }
        />
      </div>
        ...
        <div className="card" style={{ padding: 20 }}>
          <div className="field" style={{ marginBottom: 18 }}>
            ...
            <input
              ...
              defaultValue="checkout web"
              aria-invalid="true"
              style={{ borderColor: 'var(--danger)' }}
            />
            <span className="err">
              <Icon name="alert" size={11} /> Must be lowercase, alphanumeric, hyphens only.
            </span>
          </div>
          <div
            style={{
          ...
          >
            ...
            <span>
              Failed to disable <span className="mono">checkout.new-cart</span> in production —{' '}
              <span className="mono">409 Conflict</span>. Someone else just edited this flag.
            </span>
