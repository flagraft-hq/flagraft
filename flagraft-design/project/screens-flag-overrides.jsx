...
function ContextOverridesSection({ flag, defaults }) {
  ...
  const envOverrides = useMemo(() => overrides.filter((o) => o.env === env), [overrides, env])
  ...
  function addOverride(payload) {
    ...
    toast.push({
      ...
      msg: `${payload.key} ${payload.op} ${payload.val} → ${payload.result ? 'on' : 'off'}`,
    })
  }
  ...
  return (
    <section className="ctx-ovr-section">
      ...
      {envOverrides.length === 0 && !adding ? (
        <EmptyOverrides envSlug={env} defaultOn={envDefault} onAdd={() => setAdding(true)} />
      ) : (
...
function OverrideRow({ index, override, onEdit, onDelete }) {
  ...
  const opLabel =
    (OPS_BY_TYPE[field?.type] || OPS_BY_TYPE.string).find((o) => o[0] === override.op)?.[1] ||
    override.op
  return (
      ...
      <div className="rule">
        ...
        <span className="val mono">
          {override.val.includes(',') ? (
            <span>{override.val}</span>
          ) : (
            <>&ldquo;{override.val}&rdquo;</>
          )}
        </span>
      ...
      <div className="meta">
        ...
        <Tip tip="Approximate match count from the last 7 days of evaluations">
          <span className="match mono">
            <Icon name="user" size={11} />~{matches.toLocaleString()} matched
          </span>
        </Tip>
      </div>
        ...
        {conflict ? (
          <div className="form-msg warn">
            ...
            <span>
              ...
              returns the opposite result. The earlier one will win.
            </span>
          </div>
        ) : null}
        {!inRegistry ? (
          <div className="form-msg warn">
            <Icon name="alert" size={12} />
            <span>
              <span className="mono">{key}</span> isn't a registered context field. The SDK will
        ...
        {val.trim() && !duplicate ? (
          <div className="form-msg info">
            ...
            <span>
              ...
              they'll see{' '}
              <span className={'inline-state ' + (result ? 'on' : 'off')}>
                {result ? 'ON' : 'OFF'}
              </span>{' '}
  ...
  return (
    <div className="ctx-ovr-empty">
      ...
      <p>
        ...
        <span className={'inline-state ' + (defaultOn ? 'on' : 'off')}>
          {defaultOn ? 'ON' : 'OFF'}
        </span>
