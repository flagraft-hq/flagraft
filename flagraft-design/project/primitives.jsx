...
function Icon({ name, size = 16, className = '', ...rest }) {
  ...
  const P = {
    ...
    cmd: (
      <>
        <path d="M15 9V6a3 3 0 1 1 3 3h-3zM15 9v6M15 15h3a3 3 0 1 1-3 3v-3zM15 15H9M9 15v3a3 3 0 1 1-3-3h3zM9 15V9M9 9H6a3 3 0 1 1 3-3v3z" />
      </>
    ),
    code: (
      <>
    ...
    chevDouble: (
      <>
        <path d="m11 17 5-5-5-5M6 17l5-5-5-5" />
      </>
    ),
    book: (
      <>
    ...
    wifiOff: (
      <>
        <path d="M2 2l20 20M8.5 16.5a5 5 0 0 1 7 0M5 12.5a10 10 0 0 1 5.2-2.7M2 8.8a15 15 0 0 1 4.2-2.6M22 8.8A15 15 0 0 0 12 5c-.8 0-1.6.1-2.3.2" />
  ...
  return <svg {...props}>{P[name] || P.flag}</svg>
}
...
function Modal({ open, onClose, children, size, className = '' }) {
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div
        className={['modal', size, className].filter(Boolean).join(' ')}
...
function Anno({ n, top, left, right, bottom }) {
  return (
    <span className="anno-dot" style={{ top, left, right, bottom }}>
      {n}
    </span>
  )
}
