/* global React, ReactDOM,
   ...
   SettingsScreen, DesignSystemScreen, ToastProvider, useTweaks, TweaksPanel, TweakSection, TweakRadio, TweakToggle, TweakColor */
...
function App() {
  ...
  const handleNav = useCallback((id) => {
    if (id.startsWith('flag:')) {
      setRoute({ id: 'flag-detail', flagKey: id.split(':')[1] })
      return
    }
    if (id === 'flags:new') {
      setRoute({ id: 'flags' })
    ...
    if (id === 'overrides:new') {
      setRoute({ id: 'flag-detail', flagKey: window.FLAGS[0].key })
      return
    }
    if (id === 'keys:new') {
      setRoute({ id: 'keys' })
  ...
  if (route.id === 'flags')
    body = (
      <FlagsList
        activeEnv={activeEnv}
        onOpenFlag={(k) => setRoute({ id: 'flag-detail', flagKey: k })}
        openNew={openNew}
        setOpenNew={setOpenNew}
        showAnnotationsInitial={tweaks.showAnnotations}
      />
    )
  else if (route.id === 'flag-detail')
    body = <FlagDetail flagKey={route.flagKey} onBack={() => setRoute({ id: 'flags' })} />
  else if (route.id === 'overrides')
    body = <FlagDetail flagKey={'checkout.new-cart'} onBack={() => setRoute({ id: 'flags' })} />
    ...
    body = <OnboardingScreen onDone={() => setRoute({ id: 'flags' })} />
  ...
  return (
    <ToastProvider>
      <div className="app">
        ...
        />
        <SideNav
          current={route.id === 'flag-detail' ? 'flags' : route.id}
  ...
  return (
    <div style={{ display: 'grid', placeItems: 'center', height: '60vh' }}>
      <div className="card" style={{ padding: 36, textAlign: 'center' }}>
        <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600, textTransform: 'capitalize' }}>
          {title}
        </h2>
        <p className="muted" style={{ marginTop: 6 }}>
          This screen isn't part of the prototype scope.
        </p>
      </div>
    </div>
  )
