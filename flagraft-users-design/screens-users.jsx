...
function LoginScreen({ onSignedIn }) {
  ...
  const submit = (e) => {
    ...
    if (mode === 'magic') {
      setSubmitting(true);
      setTimeout(() => { setSubmitting(false); setMagicSent(true); }, 600);
      return;
    }
    if (mode === 'sso') {
      setSubmitting(true);
      setTimeout(() => { setSubmitting(false); onSignedIn?.(); }, 900);
      return;
    }
    // password
    if (!password) { setError({ field: 'password', msg: 'Enter your password.' }); return; }
    ...
    <div className="auth-page">
      ...
      <aside className="auth-brand">
        <div className="auth-brand-inner">
          <div className="auth-brand-mark">
            <span className="brand-mark" style={{ width: 40, height: 40, borderRadius: 12, fontSize: 15 }}>FR</span>
            <span style={{ fontSize: 17, fontWeight: 700, letterSpacing: '-0.01em' }}>Flagraft</span>
          </div>

          <div className="auth-pitch">
            <h1>Feature flags<br/>without the&nbsp;ceremony.</h1>
            <p>Self-hosted. Three environments out of the box. Sub-50ms evaluations from any region.</p>
          </div>
          ...
          <div className="auth-stats">
            ...
            <div>
              <div className="num">98<span className="unit">%</span></div>
              <div className="lbl">cache&nbsp;hit · last 24h</div>
            </div>
            <div>
              <div className="num">42<span className="unit">ms</span></div>
          ...
          <div className="auth-status">
            ...
            <span className="muted" style={{ marginLeft: 'auto', fontSize: 11.5, fontFamily: 'var(--font-mono)' }}>v1.4.2</span>
          </div>
        </div>
      </aside>
      ...
      <main className="auth-main">
        <div className="auth-topbar">
          <span className="muted" style={{ fontSize: 12.5 }}>New to Flagraft?</span>
          <Button size="sm" variant="ghost" rightIcon="arrowRight">Create workspace</Button>
        </div>

        <div className="auth-form-wrap">
          <div className="auth-form">
            <header className="auth-form-head">
              <h2>{mode === 'magic' ? 'Email me a sign-in link' : mode === 'sso' ? 'Continue with SSO' : 'Sign in to Flagraft'}</h2>
              <p className="muted">
                {mode === 'magic'
                  ? 'We’ll send a one-time link that expires in 10 minutes.'
                  : mode === 'sso'
                  ? ssoDomain ? <>Your workspace uses <b style={{ color: 'var(--text-1)' }}>{ssoDomain.method}</b> through {ssoDomain.name}.</> : 'Enter your work email to be redirected.'
            ...
            {magicSent ? (
              <div className="auth-magic-sent">
                <div className="ill"><Icon name="check" size={22}/></div>
                <h3>Check <span className="mono">{email}</span></h3>
                <p className="muted">If an account exists, you’ll receive a sign-in link shortly. The link expires in 10 minutes.</p>
                <div className="row" style={{ justifyContent: 'center', marginTop: 12 }}>
                  <Button variant="ghost" onClick={() => { setMagicSent(false); setMode('password'); }}>Use password instead</Button>
                  <Button variant="ghost" onClick={() => { setMagicSent(false); }}>Resend</Button>
                </div>
              </div>
            ) : (
              <form onSubmit={submit} noValidate>
                ...
                <div className="auth-sso-stack">
                  <button type="button" className="auth-sso-btn" onClick={() => { setMode('sso'); setSubmitting(true); setTimeout(() => { setSubmitting(false); onSignedIn?.(); }, 800); }}>
                    <GoogleG/>
                  ...
                  <button type="button" className="auth-sso-btn" onClick={() => { setMode('sso'); setSubmitting(true); setTimeout(() => { setSubmitting(false); onSignedIn?.(); }, 800); }}>
                    ...
                    {ssoDomain ? <span className="auth-sso-meta mono">{ssoDomain.name.toLowerCase()}.com</span> : null}
                  </button>
                </div>
                  ...
                  <div className="auth-input-wrap">
                    ...
                    {ssoDomain ? (
                      <Tip tip={`This workspace uses ${ssoDomain.method}`}>
                        <span className="auth-sso-pill"><Icon name="shield" size={10}/> sso</span>
                      </Tip>
                    ) : null}
                  </div>
                </div>
                ...
                {mode === 'password' ? (
                  <div className="field">
                    <label htmlFor="auth-pw" style={{ display: 'flex' }}>
                      ...
                      <a href="#" onClick={(ev) => { ev.preventDefault(); }} className="auth-link">Forgot?</a>
                    </label>
                    <div className="auth-input-wrap">
                      ...
                      <input
                        ...
                        style={error?.field === 'password' ? { borderColor: 'var(--danger)' } : undefined}
                      />
                      <button type="button" className="auth-input-toggle" onClick={() => setShowPw((v) => !v)} aria-label={showPw ? 'Hide password' : 'Show password'}>
                        <Icon name={showPw ? 'eyeOff' : 'eye'} size={14}/>
                      </button>
                    </div>
                    {capsOn ? <span className="hint" style={{ color: 'var(--acc-fg)' }}><Icon name="alert" size={11}/> Caps Lock is on</span> : null}
                    {error?.field === 'password' ? (
                      <span className="err"><Icon name="alert" size={11}/> {error.msg}</span>
                    ) : null}
                  </div>
                ) : null}
                ...
                {mode === 'password' ? (
                  <div className="row auth-remember">
                    <Toggle checked={remember} onChange={setRemember} size="sm" ariaLabel="Remember this device"/>
                    <span style={{ fontSize: 12.5, color: 'var(--text-2)' }}>Remember this device for 30 days</span>
                    <span className="spacer"/>
                    <Tip tip="2FA is required by your workspace policy">
                      <span className="badge teal" style={{ fontSize: 10.5 }}><Icon name="shield" size={10}/> 2FA</span>
                    </Tip>
                  </div>
                ) : null}
                ...
                <div className="auth-altmode">
                  {mode === 'password' ? (
                    <button type="button" className="auth-link" onClick={() => { setMode('magic'); setError(null); }}>
                    ...
                    <button type="button" className="auth-link" onClick={() => { setMode('password'); }}>
                      <Icon name="arrowRight" size={12} style={{ transform: 'rotate(180deg)' }}/> Use password instead
                    </button>
                  )}
                </div>
              </form>
            ...
            <footer className="auth-form-foot">
              <span className="mono" style={{ fontSize: 11 }}>Self-hosted · <span style={{ color: 'var(--pri-fg)' }}>flagraft.kocharsoft.internal</span></span>
              <span className="spacer"/>
              <a href="#" className="auth-link" onClick={(e) => e.preventDefault()}>Docs</a>
              <a href="#" className="auth-link" onClick={(e) => e.preventDefault()}>Status</a>
