...
const NAV = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'users', label: 'Users', icon: 'user', shortcut: ['g', 'u'], count: 12 },
  ...
  },
  {
    group: 'EXPLORE',
    items: [
      { id: 'login', label: 'Login screen', icon: 'shield' },
  ...
  const items = useMemo(() => {
    const all = [
      ...
      },
      {
        ...
        label: 'Environments',
        kbd: 'g e',
        run: () => onNav('environments'),
      },
      { group: 'Go to', icon: 'key', label: 'API keys', kbd: 'g k', run: () => onNav('keys') },
      { group: 'Go to', icon: 'user', label: 'Users', kbd: 'g u', run: () => onNav('users') },
      {
        group: 'Go to',
        icon: 'history',
        label: 'Audit log',
      ...
      { group: 'Go to', icon: 'shield', label: 'Login screen', run: () => onNav('login') },
      {
        group: 'Actions',
        icon: 'plus',
        label: 'New flag',
      ...
      { group: 'Actions', icon: 'plus', label: 'Invite user', run: () => onNav('users:invite') },
      ...window.FLAGS.map((f) => ({
        group: 'Flags',
        icon: 'flag',
        label: f.name,
        meta: f.key,
        run: () => onNav('flag:' + f.key),
      })),
    ]
            ...
            return (
              <React.Fragment key={i}>
                ...
                >
                  ...
                  {it.kbd ? (
                    <span className="meta">
                      <Kbd>{it.kbd.split(' ')[0]}</Kbd>
                      {it.kbd.split(' ')[1] ? <Kbd>{it.kbd.split(' ')[1]}</Kbd> : null}
                    </span>
                  ) : null}
                </div>
              </React.Fragment>
  ...
  const groups = [
    ...
    },
    {
      ...
      rows: [
        ['New flag', ['n']],
        ['Filter', ['/']],
        ['Toggle selected (dev)', ['1']],
        ['Toggle selected (stg)', ['2']],
        ['Toggle selected (prod)', ['3']],
      ...
      <div className="modal-body" style={{ padding: '8px 12px 16px' }}>
        {window.PROJECTS.map((p) => (
          ...
          >
            ...
            <span className="meta num" style={{ marginLeft: 8 }}>
              {p.flagCount} flags
            </span>
          </button>
        ))}
